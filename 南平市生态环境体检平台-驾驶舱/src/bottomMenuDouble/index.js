window.bottomMenuDoubleComponent = {
template: 
`<div id="t-l-c-bottomMenuDouble" class="t-l-c-bottomMenuDouble" >` +
...
Object.keys(state.menuList).forEach(key =>{
  if(state.menuList[key]?.isActive !== undefined){
...
return new Promise((resolve, reject) => {
  ...
  const isGeeentheme = global.activeTheme === '绿色主题'
  // 要跳过的顶层节点 RES_ID
  const skipRootId = "1a0c3796-f8ba-410c-9d4b-c643a9b606d8";

  // 构建映射，便于查找
  const map = {};
  res?.data.forEach(item => {
    map[item.RES_ID] = item;
  });

  // 找出所有 PARENT 是 skipRootId 的节点 → 这些将成为新的根菜单
  const newRoots = res?.data?.filter(item => item.PARENT === skipRootId);

  // 构建最终结构
  const arr = newRoots.map(item => {
    // 查找该节点的所有子节点
    const children = res?.data?.filter(i => i.PARENT === item.RES_ID);
...
state.menuList.data4 = _.cloneDeep(state.menuList.data3)
state.menuList.data3 = {}
debugger
// Object.keys(state.menuList).forEach(key =>{
...
setIcon()
 ...
 function handleWindowResize() {
                            setPageScale('t-l-c-bottomMenuDouble', global.appScaleMode, 'normal');
