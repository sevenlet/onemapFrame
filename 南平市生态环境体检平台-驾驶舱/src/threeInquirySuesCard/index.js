/**
 * 自动编译的Vue组件: threeInquirySuesCardComponent
...
(function () {
  const componentOptions = {
    ...
    setup(props, { emit }) {
      ...
      const cardList = inject('cardListData') ?? [];
      return {
        cardList,
      };
    },
          ...
          renderList(_ctx.cardList, (item) => {
            return (
              ...
              createElementBlock(
                ...
                [
                  item.title === '牵头单位' || item.title === '整改责任单位'
  ...
  async function loadComponent(pageCode) {
    if (!window[`threeInquirySuesCardComponent`]) {
      try {
        await window.ComponentLoader.loadComponent(pageCode);
      } catch (error) {
        console.error(`加载依赖组件 threeInquirySuesCardComponent 失败:`, error);
        // 返回一个兜底组件
        window[`threeInquirySuesCardComponent`] = {
