window.ecologicalCloudCityPopComponent = {
template: 
`<div id="t-l-c-ecologicalCloudCityPop" class="t-l-c-ecologicalCloudCityPop" >` +
`<t-rect id="t-rect-ea15.21d707542" class="ecologicalCloudCityPop-t-rect-0-0"  :tip="tRectx0x0ComputedData.tip" :label="tRectx0x0ComputedData.label" :active="tRectx0x0ComputedData.active" :c-style="tRectx0x0ComputedData.cStyle">` +
`</t-rect>` +
`<t-list-card id="t-list-card-ce0a.d4fe3b3f6" class="ecologicalCloudCityPop-t-list-card-0-1"  :data="state.detailData" :spacing-x="tListCardx0x1ComputedData.spacingX" :col="tListCardx0x1ComputedData.col" :direction="tListCardx0x1ComputedData.direction" :exchange-order="tListCardx0x1ComputedData.exchangeOrder" :unit-break="tListCardx0x1ComputedData.unitBreak" :key-field="tListCardx0x1ComputedData.keyField" :active-key-field="tListCardx0x1ComputedData.activeKeyField" :active="tListCardx0x1ComputedData.active" :c-style="tListCardx0x1ComputedData.cStyle">` +
`</t-list-card>` +
`<t-text id="t-text-80b9.1bda636ca" class="ecologicalCloudCityPop-t-text-0-2"  :label="state.pointName" :editable="tTextx0x2ComputedData.editable" :c-style="tTextx0x2ComputedData.cStyle">` +
`</t-text>` +
`</div>` +
'',
setup() {
...
const state = Vue.reactive({
   screenSize: "",
   REGION_CODE: "",
   zwfwData: [{"title":"新增行政事项","num":"--","unit":"个"},{"title":"创建资源目录数量","num":"--","unit":"个"},{"title":"上传共享数据集","num":"--","unit":"个"},{"title":"公开共享数据集","num":"--","unit":"个"}],
...
const assignData = (targetData) => {
  ...
  targetData[3].num = params.DATA_4;
  return targetData;
};
...
const businessTypeHandlers = {
  'hyhj': () => {
    getHyhjFloatGarbageSectionData();
    getHyhjOffshoreAreaPointData();
    getHyhjExcellentRate();
    return state.hyhjData;
  },
  'trhj': () => {
    getTrhjConstructCountData();
    getTrhjFarmingCountData();
    getTrhjFarmingPercentData();
    return state.trhjData;
  },
  'pwxk': () => assignData(state.pwxkData),
  'xfts': () => assignData(state.xfjbData),
  'hjzf': () => assignData(state.hjzfData),
  'zwfw': () => assignData(state.zwfwData)
};
...
getTrhjConstructPercentDataCopy();
 function handleWindowResize() {
                            setPageScale('t-l-c-ecologicalCloudCityPop', global.appScaleMode, 'normal');
