...
function mountDynamicComponent(
  ...
  if (options.style && typeof options.style === 'object') {
    ...
    console.log('mountDynamicComponent style keys:', styleKeys, 'options.style type:', Object.prototype.toString.call(options.style));
    styleKeys.forEach(key => {
      // 跳过数字索引（CSSStyleDeclaration 的只读属性）
      if (!isNaN(key)) return;
      // 避免设置 CSSStyleDeclaration 的只读属性
      try {
...
async function fetchApiData(apiRegistry, comState, apiDataCache, apiName, componentPropBindingMap, forceRefresh = false, componentId, callBack) {
  ...
  // 检查缓存
  if (!forceRefresh && localApiDataCache[cacheKey]) {
...
function generateCacheKey(apiName, params) {
  ...
  return `${apiName}_${btoa(paramStr).replace(/[+/=]/g, '')}`; // 使用base64编码避免特殊字符
}
...
export {
  ...
  loadAndRegisterComponentAndRoute,
  ResourceLoader,
  ComponentLoader,
  replaceCssVariables,
  loadInitialApis,
