import { AQI, convertCharacter } from '@/utils.js';

window.setDomHeightValue = (domId,heightValue) => {
/**
 * 设置dom元素的高度值
 * domId: 元素id选择器值
...
window.getForwardTimeList = (timeValue,forwardType,forwardCount) => {
/**
 * 往前推时间（包括当前时间）
 * timeValue: 当前时间值
...
window.getRegionLevel = (selectRegionCode) => {
/**
 * 获取行政区等级
...
window.getWaterLevelColor = (value) => {
let color = '';
// 水质级别
...
window.dateFormat = (date,sFormat) => {
/**
 * 日期对象转为日期字符串
...
window.getCenterIndexList = (moduleType,timeType) => {
/**
...
window.setDomVisible = (domId,isVisible) => {
/**
 * domId：元素id选择器值
...
window.setDomLeftValue = (domId,leftValue) => {
/**
 * domId: 元素id选择器值
...
window.setDomTopValue = (domId,topValue) => {
/**
 * domId: 元素id选择器值
...
window.handleStringSplit = (strValue,separator,arrLength) => {
/**
 * @description: 处理字符串分割
 * @param strValue 值
...
window.replacePollutionCharacter = (optionString) => {
/**
...
window.handleTooltipFormatter = (formatterParams) => {
...
let str = '';
let name = formatterParams[0].data?.tooltipXAxisLabel || '--';
...
window.handleStandardSeries = (seriesName,lineColor,labelPosition) => {
// 默认线条颜色
...
window.getUrlParamValue = (paramName) => {
...
window.closeWarningFilterPanel = () => {
};
window.getTwoDaysBefore = (dateInput) => {
...
window.getUrlQueryString = (paramName) => {
...
window.formPointTree = (originalData,keynote,backtype) => {
...
window.judgeTimesOverDays = (startTime,endTime,count) => {
// 一天的毫秒数
...
window.setEleWidthHeight = (id,width,height) => {
try {
...
window.setEleLeftTop = (id,left,top) => {
try {
  const element = document.getElementById(id);
  if (!element) return;
  // 可能只改top或left
