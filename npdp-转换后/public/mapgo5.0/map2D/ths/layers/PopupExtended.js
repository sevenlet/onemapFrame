    ...
    return declare([Popup], {
        constructor: function (options) {
            ...
            */
            ...
            //设置拓展属性
            this.themeClass = options.extended.themeClass||"extended";//主题
            ...
            this.smallStyleWidthBreak = options.extended.smallStyleWidthBreak || 500;

            //宋永菊添加的参数
            this.isFollowMap=options.extended.isFollowMap||true;
        ...
        setMap: function (map) {
            ...
            this._setPopupMode();
            dojo.destroy(this.domNode);//销毁domNode节点
            return this.inherited(arguments);//调用父类中同名（setMap）的方法
        },
        ...
        show: function (popupInfo, options) {
            ...
            for (var i = 0, len = this.openPopups.length; i < len; i++) {
                ...
                //检查是否允许一次打开多个窗口，如果不被允许的话，关闭所以已打开的窗口关闭旧的，打开新的
                //检查这个popup的features是否已经匹配上了其他的任何的打开的popup，如果已经有了，则
            ...
            //由宋永菊修改的地方
            // this.features=graphicsObject(this.graphics);
            //
            var popup = new Popup(this.options, dojo.create("popupDiv"));
            popup.setFeatures(this.features);
            if(popupInfo.title)
            {
                popup.setTitle(popupInfo.title);
            }
            if(popupInfo.content)
            {
                popup.setContent(popupInfo.content);
            }
            if(popupInfo.width&&popupInfo.width)
            {
                popup.resize(popupInfo.width, popupInfo.height);
            }
            ...
            popup.show(popupInfo.location, options);
        ...
        _mapMouseMove: function (e) {
            ...
            if (this.map.popupDragging) {
                ...
                if (pu.isSnapped) {
                    this._addResizer(pu); //add resizing controls 添加调整大小的控制
                    if(!this.isFollowMap)
                    {
                        pu._unfollowMap(); //阻止窗口随着地图的平移和缩放而移动
                    }
        ...
        _addTemplateOptions: function (popup) {
            ...
            var templateOptions = template.extended || {};
            ...
            if (templateOptions.themeClass) {
                dojo.addClass(popup.domNode, templateOptions.themeClass);//给popup的指定节点设置样式
            }
