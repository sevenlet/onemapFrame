...
var Proxy = exports.Proxy = function Proxy(src, options) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        xhr.onerror = reject;
        xhr.open('GET', proxy + '?' + src + '&responseType=' + responseType);
...
var DocumentCloner = exports.DocumentCloner = function () {
    ...
    _createClass(DocumentCloner, [{
        ...
        key: 'cloneNode',
        value: function cloneNode(node, parentNode) {
            var clone = node.nodeType === Node.TEXT_NODE ? document.createTextNode(node.nodeValue) : this.createElementClone(node);
            // 元素克隆后的回调
            if (this.options.onCloneNodeCallback) {
				this.options.onCloneNodeCallback(clone, parentNode);
			}
            ...
            for (var child = node.firstChild; child; child = child.nextSibling) {
                ...
                !this.options.ignoreElements(child))) {
                    if (!this.copyStyles || child.nodeName !== 'STYLE') {
                        clone.appendChild(this.cloneNode(child, clone));
                    }
                }
            }
        ...
        value: function addImage(key, src, useCORS) {
            ...
            var imageLoadHandler = function imageLoadHandler(supportsDataImages) {
                return new Promise(function (resolve, reject) {
                    ...
                    // 是否遵循图片本身的 complete 属性来处理，当需要自行加载资源时应设置为 false
					          var isProcessComplete = true;

                    // 处理地图普通图片点位打印不出来的问题
                    if (src.indexOf('data:image/svg+xml,') > -1) {
                    	/* 确定是否有图片 */
                    	var svgStr = decodeURIComponent(src);
                    	const regex = /xlink:href="([^"]+)"/g;
                    	const matches = {};
                    	let match;
                    	while ((match = regex.exec(svgStr)) !== null) {
                    		matches[match[1]] = match[1]; // match[1] 是捕获的地址
                    	}
                    	if (Object.keys(matches).length) {
                        isProcessComplete = false;
                    		// 请求所有的图片
                    		const imagePromises = [];
                    		Object.keys(matches).forEach((matchImagePath) => {
                    			if (_this4.cache[matchImagePath]) {
                    				return;
                    			}
                    			_this4.cache[matchImagePath] = new Promise((fetchResolve, fetchReject) => {
                    				fetch(matchImagePath).then((res) => res.blob()).then((blobRes) => {
                                                  // 解析数据
                    					const reader = new FileReader();
                    					reader.readAsDataURL(blobRes);
                    					reader.onloadend = () => {
                                                      // 返回 base64 格式的图片
                    						_this4.cache[matchImagePath] = reader.result;
                    						fetchResolve(reader.result);
                    					};
                    					reader.onerror = fetchReject;
                    				});
                    			});
                    			imagePromises.push(_this4.cache[matchImagePath]);
                    		});

                    		Promise.all(imagePromises).then(() => {
                    			Object.keys(matches).forEach((matchImagePath) => {
                    				svgStr = svgStr.replace(new RegExp(matchImagePath, 'gm'), _this4.cache[matchImagePath]);
                    			});
                    			img.src = svgStr;
                    		});
                    	} else {
                    ...
                    if (isProcessComplete && img.complete === true) {
