...
(function() {
  ...
  function geoJsonConverter(){
    ...
    function gcGeometryToEsriGeometry(gcGeom, wkid) {
      var esriGeometry,
        esriGeomInfo,
    ...
    function gcFeatureToEsriFeature(gcFeature,wkid) {
      var esriFeat,
        prop,
        esriAttribs;
      if (gcFeature) {
        ...
        if (gcFeature.geometry) {
          esriFeat.geometry = gcGeometryToEsriGeometry(gcFeature.geometry,wkid);
        }
    ...
    gCon.toEsri = function(geoJsonObject, wkid) {
      var outObj,
        i,
        gcFeats,
        esriFeat;
      if (geoJsonObject){
        if (geoJsonObject.type === "FeatureCollection"){
          ...
          for (i = 0; i < gcFeats.length; i++) {
            esriFeat = gcFeatureToEsriFeature(gcFeats[i], wkid);
            if (esriFeat) {
          ...
          outObj = gcFeatureToEsriFeature(geoJsonObject, wkid);
        }
        else{
          outObj = gcGeometryToEsriGeometry(geoJsonObject, wkid);
        }
      }
      return outObj;
    };
