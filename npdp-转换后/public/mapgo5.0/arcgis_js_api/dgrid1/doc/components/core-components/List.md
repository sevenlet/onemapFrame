...
`resizeThrottleDelay` | The delay (in milliseconds) passed to the `resizeThrottleMethod`. A lower value will provide more responsive grid resizing. If there are a large number of grids on the page, a higher value can improve performance (or specify 'debounce' for `resizeThrottleMethod`).<br>Default: 15
`resizeThrottleMethod` | Can be a string or function.<br>String: the name of a method ('debounce', 'throttle', 'throttleDelayed') from `dgrid/util/misc` to throttle or debounce the window resize handler.<br>Function: a function to throttle or debounce the window resize handler. The function will receive two parameters:<br>`callback` (Function): the function to be throttled<br>`delay` (Integer): the value of the `resizeThrottleDelay` property<br>The function must return a function that executes the callback function.<br>Default: 'throttleDelayed'
