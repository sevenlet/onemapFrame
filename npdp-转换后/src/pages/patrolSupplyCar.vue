...
const state = reactive({
   screenSize: "",
   componentName: "patrolSupplyCarContentComponent",
});
// 选中的图表主题
...
name:"patrolSupplyCarContentComponent",
loading:true,
...
if (tabValue !== state.tabSelect) {
 ...
 function handleWindowResize() {
                            setPageScale('t-l-c-patrolSupplyCar', global.appScaleMode, 'normal');
...
handleWindowResize();
/* 接收socket.io消息 */
const onSocketChangedcc96d1aa9f04d53a2f7 = function (data) {
...
};rootData.rootOn('rootSocket:change', onSocketChangedcc96d1aa9f04d53a2f7);onUnmounted(() => {
  rootData.rootOff('rootSocket:change', onSocketChangedcc96d1aa9f04d53a2f7);
});
/* 接收socket.io消息 */
const onSocketChange142503e64bfc46ceab4b = function (data) {
...
};rootData.rootOn('rootSocket:change', onSocketChange142503e64bfc46ceab4b);onUnmounted(() => {
  rootData.rootOff('rootSocket:change', onSocketChange142503e64bfc46ceab4b);
});
/* 接收socket.io消息 */
const onSocketChangee852becbb2c84681b1c9 = function (data) {
if(data.type === "showTrajectorySupplySprinkler"){
try{
// 设置变量值
state.componentName = 'mobileSurveySupplyDialogComponent';
} catch (error) {
console.error('参数格式错误', error);
};
};
};rootData.rootOn('rootSocket:change', onSocketChangee852becbb2c84681b1c9);onUnmounted(() => {
  rootData.rootOff('rootSocket:change', onSocketChangee852becbb2c84681b1c9);
});
/* 接收socket.io消息 */
const onSocketChange54139b9fcf98475eaa4d = function (data) {
if(data.type === "closeTrajectorySupplySprinkler"){
try{
// 设置变量值
state.componentName = 'patrolSupplyCarContentComponent';
} catch (error) {
console.error('参数格式错误', error);
};
};
};rootData.rootOn('rootSocket:change', onSocketChange54139b9fcf98475eaa4d);onUnmounted(() => {
  rootData.rootOff('rootSocket:change', onSocketChange54139b9fcf98475eaa4d);
});
});
// 页面离开时的操作
onUnmounted(() => {
...
.t-l-c-patrolSupplyCar .patrolSupplyCar-t-component-0-0 {
