/**
 * 自动编译的Vue组件: wzxcCarSliderComponent
...
(function () {
  ...
  async function loadComponent(pageCode) {
    if (!window[`wzxcCarSliderComponent`]) {
      try {
        await window.ComponentLoader.loadComponent(pageCode);
      } catch (error) {
        console.error(`加载依赖组件 wzxcCarSliderComponent 失败:`, error);
        // 返回一个兜底组件
        window[`wzxcCarSliderComponent`] = { template: '<div>Failed to load component</div>' };
      }
    }
  }

  // 注册组件到全局
  window.wzxcCarSliderComponent = {
    ...componentOptions,
    render,
  };
})();
