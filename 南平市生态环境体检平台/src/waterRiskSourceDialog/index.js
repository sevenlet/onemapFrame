window.waterRiskSourceDialogComponent = {
template: 
`<div id="t-l-c-waterRiskSourceDialog" class="t-l-c-waterRiskSourceDialog" >` +
`<t-rect id="t-rect-8252.1a4ade4f4" class="waterRiskSourceDialog-t-rect-0-0"  :tip="tRectx0x0ComputedData.tip" :label="tRectx0x0ComputedData.label" :active="tRectx0x0ComputedData.active" :c-style="tRectx0x0ComputedData.cStyle">` +
`</t-rect>` +
`<t-rect id="t-rect-e3a7.d7772ec8d" class="waterRiskSourceDialog-t-rect-0-1"  :tip="tRectx0x1ComputedData.tip" :label="tRectx0x1ComputedData.label" :active="tRectx0x1ComputedData.active" :c-style="tRectx0x1ComputedData.cStyle">` +
`</t-rect>` +
`<t-text id="t-text-8cba.b16affff7" class="waterRiskSourceDialog-t-text-0-2"  :label="state.title" :editable="tTextx0x2ComputedData.editable" :c-style="tTextx0x2ComputedData.cStyle">` +
`</t-text>` +
`<t-rect id="t-rect-da62.857f90ab" class="waterRiskSourceDialog-t-rect-0-3"  :tip="tRectx0x3ComputedData.tip" :label="tRectx0x3ComputedData.label" :active="tRectx0x3ComputedData.active" :c-style="tRectx0x3ComputedData.cStyle" @click="onclick1733817210174">` +
`</t-rect>` +
`<t-text id="t-text-738d.67fdf6baf" class="waterRiskSourceDialog-t-text-0-4"  :label="tTextx0x4ComputedData.label" :editable="tTextx0x4ComputedData.editable" :c-style="tTextx0x4ComputedData.cStyle">` +
`</t-text>` +
`<t-list-card id="t-list-card-1afd.46c2cc26d" class="waterRiskSourceDialog-t-list-card-0-5"  :data="state.cardList" :spacing-x="tListCardx0x5ComputedData.spacingX" :col="tListCardx0x5ComputedData.col" :direction="tListCardx0x5ComputedData.direction" :exchange-order="tListCardx0x5ComputedData.exchangeOrder" :unit-break="tListCardx0x5ComputedData.unitBreak" :key-field="tListCardx0x5ComputedData.keyField" :active-key-field="tListCardx0x5ComputedData.activeKeyField" :active="tListCardx0x5ComputedData.active" :c-style="tListCardx0x5ComputedData.cStyle">` +
`</t-list-card>` +
`<t-row id="t-row-1aec.588d3a901" class="waterRiskSourceDialog-t-row-0-6"  :c-style="tRowx0x6ComputedData.cStyle" :gutter="tRowx0x6ComputedData.gutter" :justify="tRowx0x6ComputedData.justify" :align="tRowx0x6ComputedData.align">` +
`<t-text id="t-text-0df0.1c50452" class="waterRiskSourceDialog-t-text-0-6-0"  :label="tTextx0x6x0ComputedData.label" :editable="tTextx0x6x0ComputedData.editable" :c-style="tTextx0x6x0ComputedData.cStyle" @click="onclick1734499551425">` +
`</t-text>` +
`</t-row>` +
`</div>` +
'',
...
const state = Vue.reactive({
   screenSize: "",
   point: {"lon":"","lat":""},
   cardList: [{"title":"行政区","num":"-"},{"title":"风险源类型","num":"-"},{"title":"是否有应急预案","num":"-"}],
   title: "-",
   id: "",
});
// 选中的图表主题
...
label:"",
active:false,
cStyle:{"wrapper":{"default":{"width":"450px","height":"237px","position":"unset","left":"0px","top":"0px","backgroundColor":"var(--business-dialog-bg)","borderTopLeftRadius":"16px","borderTopRightRadius":"16px","borderBottomLeftRadius":"16px","borderBottomRightRadius":"16px","borderLeft":"1px solid var(--business-show-bg60)","borderTop":"1px solid var(--business-show-bg60)","borderRight":"1px solid var(--business-show-bg60)","borderBottom":"1px solid var(--business-show-bg60)","animationName":""}}}, 
 },
},
tRectx0x1: {
defaultStyle: {
...
tTextx0x4: {
defaultStyle: {
            default: {"width":"450px","height":"52px","color":"var(--t-font-color-gy1)","position":"unset","left":"0px","top":"185px","borderBottomLeftRadius":"16px","borderBottomRightRadius":"16px","borderLeft":"1px solid var(--business-show-bg60)","borderTop":"1px solid var(--business-show-bg60)","borderRight":"1px solid var(--business-show-bg60)","borderBottom":"1px solid var(--business-show-bg60)","backgroundColor":"var(--business-show-bg15)"},
...
tListCardx0x5: {
defaultStyle: {
            default: {"width":"410px","height":"110px","position":"unset","left":"20px","top":"64px","overflow":"auto","animationName":""},
            
            },
 default: { 
 
data:[{"title":"行政区","num":"-"},{"title":"风险源类型","num":"-"},{"title":"是否有应急预案","num":"-"}],
spacingX:0,
col:1,
direction:"vertical",
exchangeOrder:false,
...
state.id = global.businessDialog?.dialogParmas?.sid || '';
...
if (state.point.lon && state.point.lat) {
  ...
  rootData.rootSocket.emit('message', {
    ...
    data: [ // 消息内容
      ...
      { shareCode: "POINT_LOACTION_NAME", runtimeValue: state.title }
    ]
  });
...
return new Promise((resolve, reject) => {
  http({"url":global.serviceBasePath,"method":"get","baseURL":global.foshanServiceBaseUrl,"params":{"SID":state.id,"interfaceId":"461320bd9fa40b5d7fca85ba87f61b57"},}).then((res) => {
     // 成功的操作
     state.cardList.forEach(i => i.num = '-');
state.title = '-'

if (res?.status === 200 && res?.data?.data) {
  const { ENTER_NAME, CODE_REGION, ENTERPRISE_TYPES, EMERGENCY_PLAN, LATITUDE, LONGITUDE } = res.data.data
  // 风险源名称
  state.title = ENTER_NAME || '-'
  // 行政区
  state.cardList[0].num = CODE_REGION || '-';
  // 风险源类型
  state.cardList[1].num = ENTERPRISE_TYPES || '-';
  // 是否有应急预案
  state.cardList[2].num = EMERGENCY_PLAN || '-';

  // 经纬度--周边查询使用
  state.point.lat = LATITUDE || '';
  state.point.lon = LONGITUDE || '';
}
...
state.title = '-'
 ...
 function handleWindowResize() {
                            setPageScale('t-l-c-waterRiskSourceDialog', global.appScaleMode, 'normal');
